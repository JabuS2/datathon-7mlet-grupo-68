...
def _orm_to_state(orm: EstadoBraco) -> ArmState:
    return ArmState(
        arm_id=orm.arm_id, alpha=orm.alpha, beta=orm.beta,
        n_pulls=orm.n_pulls, sum_reward=orm.sum_reward, A=orm.A, b=orm.b,
    )
...
class DecisionService:
    ...
    async def decide(self, cod_cliente: int, channel) -> DecideResponse:
        async with self.uow:
            ...
            states = [_orm_to_state(s) for s in await self.uow.estados_braco.list_by_policy(policy.policy_id)]
            pol = build_policy(policy.algorithm, dimension=self.rt.stats.dimension, hyperparams=policy.hyperparams)
            ...
            dec = Decisao(
                cod_cliente=cod_cliente, policy_version=policy.policy_id,
                chosen_arm_id=decision.arm_id, channel=channel,
                context=decision.context, reason_codes=decision.reason_codes,
            ...
            return DecideResponse(
                decision_id=dec.decision_id, arm_id=decision.arm_id,
                product_name=offer["product_name"], description=offer["description"],
                category=offer["category"], channel=channel, score=_finite(float(decision.score)),
                reason_codes=decision.reason_codes, policy_version=policy.policy_id,
            )
        ...
        async with self.uow:
            ...
            items = [
                OfertaRankeada(
                    ...
                    score=_finite(float(r.score)), reason_codes=r.reason_codes, rank=i + 1,
                )
                for i, r in enumerate(ranked[:top_k])
            ]
            return ShowcaseResponse(cod_cliente=cod_cliente, policy_version=policy.policy_id, items=items)
    ...
    async def feedback(self, decision_id: UUID, event_type: TipoEvento) -> FeedbackResponse:
        async with self.uow:
            ...
            return FeedbackResponse(
                event_id=evt.event_id, decision_id=decision.decision_id,
                type=event_type, occurred_at=evt.occurred_at or datetime.now(timezone.utc),
            )

    # ── /reward ──────────────────────────────────────────────────
    async def reward(self, decision_id: UUID, converted: bool, value: float | None) -> RewardResponse:
        async with self.uow:
            decision = await self._require_decision(decision_id)
            ...
            client = await self.uow.clientes.get_by_cod_cliente(decision.cod_cliente) if decision.cod_cliente else None
            ...
            reward.observed_at = datetime.now(timezone.utc)
            await self.uow.session.flush()
            return RewardResponse(
                reward_id=reward.reward_id, decision_id=decision_id,
                value=reward_value, status=reward.status,
            )
